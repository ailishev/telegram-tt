...
class TelegramClient {
  ...
  private getSelfUser() {
    return this.mockData.users.find((user) => (user as any).self) || this.mockData.users[0];
  }

  private getBackendProfileByPeerId(peerId: string) {
    return this.mockData.backendProfileByPeerId?.[peerId];
  }

  private upsertBackendProfile(profile: {
    id: string;
    phoneNumber?: string;
    username?: string;
    firstName?: string;
    ...
    const isSelf = profile.id === this.mockData.currentProfileBackendId;
    const peerId = this.mockData.peerIdByBackendProfileId?.[profile.id] || getBackendPeerId(profile.id, isSelf);

    this.mockData.peerIdByBackendProfileId = {
      ...(this.mockData.peerIdByBackendProfileId || {}),
      [profile.id]: peerId,
    };
    this.mockData.backendProfileByPeerId = {
      ...(this.mockData.backendProfileByPeerId || {}),
      [peerId]: {
        ...(this.mockData.backendProfileByPeerId?.[peerId] || {}),
        ...profile,
      },
    };

    const existingUser = this.mockData.users.find((user) => user.id === peerId);
    const nextUser = {
      ...(existingUser || {}),
      id: peerId,
      ...(isSelf ? { self: true } : {}),
      firstName: profile.firstName || profile.username || existingUser?.firstName || 'User',
      lastName: profile.lastName || existingUser?.lastName || '',
      username: profile.username || existingUser?.username,
      phone: profile.phoneNumber || (existingUser as any)?.phone,
      verified: profile.isVerified ?? (existingUser as any)?.verified,
      premium: profile.isPremium ?? (existingUser as any)?.premium,
      bio: profile.bio ?? (existingUser as any)?.bio,
      stargiftsCount: profile.gifts?.length ?? (existingUser as any)?.stargiftsCount,
      botVerificationIcon: profile.isVerified ? 1n : (existingUser as any)?.botVerificationIcon,
    };

    if (existingUser) {
      Object.assign(existingUser, nextUser);
    } else {
      this.mockData.users.push(nextUser as any);
    }

    return peerId;
  }

  private async ensurePrivateDialog(peerId: string) {
    if (this.mockData.backendDialogByPeerId?.[peerId]) {
      return;
    }

    const profile = this.getBackendProfileByPeerId(peerId);
    if (!profile) {
      return;
    }

    await startPrivateDialog({
      targetProfileId: profile.id,
      username: profile.username,
    }).catch(() => undefined);

    if (isSupabaseConfigured()) {
      await this.loadFromSupabase().catch(() => undefined);
    }
  }
  ...
  async invoke<A, R>(request: Api.Request<A, R>) {
    ...
    if (request instanceof Api.users.GetFullUser) {
      const userId = request.id instanceof Api.InputUserSelf
        ...
        : request.id instanceof Api.InputUser
          ...
          : this.getSelfUser()?.id || '1';
      const currentUser = this.mockData.users.find((user) => user.id === userId);
      const backendProfile = currentUser ? this.getBackendProfileByPeerId(currentUser.id) : undefined;
      if (!currentUser) {
        return undefined;
      }
      ...
      return new Api.users.UserFull({
        fullUser: new Api.UserFull({
          about: backendProfile?.bio || (currentUser as any)?.bio || '',
          settings: new Api.PeerSettings({}),
          notifySettings: new Api.PeerNotifySettings({}),
          id: BigInt(currentUser?.id || '1'),
          commonChatsCount: 0,
          stargiftsCount: Number(backendProfile?.gifts?.length || (currentUser as any)?.stargiftsCount || 0),
          displayGiftsButton: Boolean(backendProfile?.gifts?.length),
        }),
        chats: [],
    ...
    if (request instanceof Api.users.GetUsers) {
      return request.id.map((inputUser) => {
        const userId = inputUser instanceof Api.InputUserSelf
          ? this.getSelfUser()?.id
          : inputUser instanceof Api.InputUser
            ? inputUser.userId.toString()
            : undefined;

        if (!userId || !this.mockData.users.find((user) => user.id === userId)) {
          return undefined;
        }

        return createMockedUser(userId, this.mockData);
      }).filter(Boolean);
    }
    ...
    if (request instanceof Api.messages.SendMessage) {
      ...
      await this.ensurePrivateDialog(peerId);
    ...
    if (request instanceof Api.contacts.Search) {
      ...
      let result = await searchBackend(normalizedQuery).catch(() => ({ users: [] }));

      if (query.startsWith('@')) {
        const exactMatch = result.users.find((profile) => normalizeUsername(profile.username) === normalizeUsername(normalizedQuery));
        if (exactMatch) {
          await startPrivateDialog({
            targetProfileId: exactMatch.id,
            username: exactMatch.username,
          }).catch(() => undefined);
          if (isSupabaseConfigured()) {
            await this.loadFromSupabase().catch(() => undefined);
          }
          result = await searchBackend(normalizedQuery).catch(() => ({ users: [] }));
        }
      }

      const peerIds = result.users.map((profile) => this.upsertBackendProfile(profile));
      const users = peerIds.map((peerId) => createMockedUser(peerId, this.mockData));
      const peers = peerIds.map((peerId) => new Api.PeerUser({ userId: BigInt(peerId) }));
    ...
    if (request instanceof Api.contacts.ResolveUsername) {
      ...
      let user = this.mockData.users.find((item) => normalizeUsername(item.username) === username);

      if (!user) {
        const result = await searchBackend(username).catch(() => ({ users: [] }));
        const matchedProfile = result.users.find((profile) => normalizeUsername(profile.username) === username);
        if (matchedProfile) {
          const peerId = this.upsertBackendProfile(matchedProfile);
          user = this.mockData.users.find((item) => item.id === peerId);
        }
      }
      ...
      return new Api.contacts.ResolvedPeer({
        peer: new Api.PeerUser({ userId: BigInt(user.id) }),
        chats: [],
    ...
    if (request instanceof Api.contacts.ResolvePhone) {
      ...
      const peerId = Object.entries(this.mockData.backendProfileByPeerId || {})
        .find(([, profile]) => profile.phoneNumber?.replace(/[^\d+]/g, '') === normalizedPhone)?.[0];
      ...
      return new Api.contacts.ResolvedPeer({
        ...
        chats: [],
        users: [createMockedUser(peerId, this.mockData)],
      });
    }
