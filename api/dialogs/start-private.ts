...
export default async function handler(req: any, res: any) {
  ...
  if (target.id === session.profileId) {
    res.status(400).json({ error: 'Cannot start private dialog with self' });
    return;
  }
  ...
  const existingDialogId = Object.entries(groupedByDialog)
    .find(([dialogId, profileIds]) => {
      const dialog = existingMemberships.find((membership) => membership.dialogId === dialogId)?.dialog;
      return dialog?.type === 'private'
        && profileIds.length === 2
        && profileIds.includes(session.profileId)
        && profileIds.includes(target.id);
    })?.[0];
